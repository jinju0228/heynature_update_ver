...
for (i = 0; i < acc.length; i++) {
  acc[i].addEventListener("click", function() {
    this.classList.toggle("no_active");
    var panel = this.nextElementSibling;
    if (panel.style.maxHeight) {
      panel.style.maxHeight = null;
    } else {
      var act = document.querySelectorAll('.accordion.no_active');//여기부터  
      for(j = 0; j<act.length; j++){
            act[j].classList.remove('no_active');
            act[j].nextElementSibling.style.maxHeight = null;
      }
      this.classList.add('no_active') //여기까지 추가함
